...
class NoteListPage extends StatefulWidget {
  final User currentUser; // Utilisateur actuellement connecté

  const NoteListPage({super.key, required this.currentUser});
...
class _NoteListPageState extends State<NoteListPage> {
  ...
  // Recharge uniquement les notes associées au userId de l'utilisateur connecté
  Future<void> _refreshNotes() async {
    setState(() => _isLoading = true);
    _notes = await DatabaseHelper.instance.getNotesByUser(widget.currentUser.id!);
    setState(() => _isLoading = false);
  }

  // Ajouter une note liée au userId connecté
  void _addNote() async {
    final text = _noteController.text.trim();
    if (text.isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'To do list (${widget.currentUser.username})',
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 20),
            // Liste des notes filtrées par utilisateur
            Expanded(
              child: _isLoading
                  ? const Center(child: CircularProgressIndicator())
                  : _notes.isEmpty
